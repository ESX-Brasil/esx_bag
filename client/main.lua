...
function Bag()
    ...
    table.insert(elements, {label = 'Colocar objeto', value = 'put'})
    table.insert(elements, {label = 'Pegar objeto', value = 'take'})
    table.insert(elements, {label = 'Soltar Bolsa', value = 'drop'})
...
function TakeItem()
    ...
    ESX.TriggerServerCallback('esx_bag:getBagInventory', function(bag)
        ...
        ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'lels',
        {
            ...
            menu.close()
            if IsWeapon then
                TriggerServerEvent('esx_bag:TakeItem', BagId, data.current.value, data.current.count, "weapon")
            else
                TriggerServerEvent('esx_bag:TakeItem', BagId, data.current.value, data.current.count, "item")
            end
        ...
        ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'lel3',
        {
            ...
            menu.close()
                ...
                if IsWeapon then
                    ...
                    TriggerServerEvent('esx_bag:PutItem', BagId, data.current.name, data.current.label2, data.current.count, "item")
                end
